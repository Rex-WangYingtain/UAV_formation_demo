...
class Actor(nn.Module):
    def __init__(self):
        ...
        # 创建一个形状为 [STEP_BACK, state_dim] 的可学习参数
        self.tanh_state_buffer_em = nn.Parameter(torch.ones(STEP_BACK, STATE_BUFFER_DIM) * TANH_EM)
        self.tanh_state_em = nn.Parameter(torch.ones(STATE_DIM) * TANH_EM)

        # LSTM 层
        self.lstm_layers = nn.LSTM(
            input_size=STATE_BUFFER_DIM, hidden_size=64, num_layers=1, batch_first=True
        )
    ...
    def forward(self, state_buffer, state):
        state_buffer = torch.tanh(self.tanh_state_buffer_em * state_buffer)
        state = torch.tanh(self.tanh_state_em * state)

        state_buffer, _ = self.lstm_layers(state_buffer)
        state_buffer = state_buffer[:, -1, :]
        state = self.linear_state(state)

        x = torch.cat([state_buffer, state], dim=-1)
...
class Critic(nn.Module):
    def __init__(self):
        ...
        self.tanh_em_state_buffer = nn.Parameter(torch.ones(STEP_BACK, STATE_BUFFER_DIM) * TANH_EM)

        self.tanh_state_em = nn.Parameter(torch.ones(STATE_DIM) * TANH_EM)

        self.tanh_em_action = nn.Parameter(torch.ones(ACTION_DIM) * TANH_EM)

        # LSTM 层
        self.lstm_layers = nn.LSTM(
            input_size=STATE_BUFFER_DIM,
            hidden_size=64,
            num_layers=1,
            batch_first=True,
        )
        ...
        self.linear_action = nn.Linear(ACTION_DIM, 32)
    ...
    def forward(self, state_buffer, state, action):
        state_buffer = torch.tanh(self.tanh_em_state_buffer * state_buffer)
        state = torch.tanh(self.tanh_state_em * state)
        action = torch.tanh(self.tanh_em_action * action)

        state_buffer, _ = self.lstm_layers(state_buffer)
        state_buffer = state_buffer[:, -1, :]
        state = self.linear_state(state)
        action = self.linear_action(action)

        x = torch.cat([state_buffer, state, action], dim=-1)
        x = self.fc(x)
...
class ReplayMemory:
    ...
    def add_memo(
        self, state_buffer, state, action, reward, next_state_buffer, next_state, done
    ):
        self.buffer.append(
            (state_buffer, state, action, reward, next_state_buffer, next_state, done)
        )
...
class TD3Agent:
    ...
    def get_action(self, state_buffer, state, add_noise):
        # 转化为符合要求的张量
        state_buffer = np.array(state_buffer)
        state_buffer_tensor = torch.FloatTensor(state_buffer).to(device=device)
        state_buffer_tensor = state_buffer_tensor.unsqueeze(0)
        ...
        # 输入获得结果
        with torch.no_grad():
            action = self.actor(state_buffer_tensor, state_tensor).squeeze().cpu().detach().numpy()
    ...
    def update_networks(self):
        ...
        next_state_buffer_samples = torch.FloatTensor(next_state_buffer_samples).to(
            device=device
        )
        ...
        # 更新Critic网络
        next_action = self.actor_target(next_state_buffer_samples, next_state_samples)
        ...
        target_Q1 = self.critic1_target(next_state_buffer_samples, next_state_samples, next_action)
        target_Q2 = self.critic2_target(next_state_buffer_samples, next_state_samples, next_action)
        target_Q = torch.min(target_Q1, target_Q2)
        target_Q = reward_samples + (GAMMA * target_Q * (1 - done_samples))

        current_Q1 = self.critic1(state_buffer_samples, state_samples, action_samples)
        current_Q2 = self.critic2(state_buffer_samples, state_samples, action_samples)
        ...
        if self.total_it % self.policy_delay == 0:
            # 更新Actor网络
            pred_action = self.actor(state_buffer_samples, state_samples)

            actor_loss = -self.critic1(state_buffer_samples, state_samples, pred_action).mean()
