...
for episode_i in range(episode_start, NUM_EPISODE):
    ...
    action1_buffer = deque(maxlen=STEP_BACK)
    for _ in range(STEP_BACK):
        state1_buffer.append(np.zeros(STATE_DIM, dtype=np.float32))
        action1_buffer.append(np.zeros(ACTION_DIM, dtype=np.float32))

    state2_buffer = deque(maxlen=STEP_BACK)
    action2_buffer = deque(maxlen=STEP_BACK)
    for _ in range(STEP_BACK):
        state2_buffer.append(np.zeros(STATE_DIM, dtype=np.float32))
        action2_buffer.append(np.zeros(ACTION_DIM, dtype=np.float32))
    ...
    for step_i in range(NUM_STEP):
        ...
        # follower1
        action1 = agent.get_action(state1_buffer, add_noise=True)
        ...
        # follower2
        action2 = agent.get_action(state2_buffer, add_noise=True)
        ...
        """计算奖励"""
        action1_buffer.append(action1)
        action2_buffer.append(action2)
