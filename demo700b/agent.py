...
from attention.SoftmaxSelfAttention import SoftmaxSelfAttention
from positionalEncoding.SinusoidalPositionalEncodingFixedLength import SinusoidalPositionalEncodingFixedLength

class MyLayer(nn.Module):
    def __init__(self, input_dim, d_model, step_back: int = 4):
        super().__init__()
        self.sinusoidalPositionalEncoding = SinusoidalPositionalEncodingFixedLength(input_dim, d_model, step_back)
        self.softmaxSelfAttention = SoftmaxSelfAttention(d_model)

    def forward(self, x):
        x = self.sinusoidalPositionalEncoding(x)
        x = self.softmaxSelfAttention(x)
        return x
...
class Actor(nn.Module):
    def __init__(self, state_dim, action_dim):
        ...
        self.tanh_em = nn.Parameter(torch.ones(STEP_BACK, state_dim) * TANH_EM)

        self.mylayer = MyLayer(input_dim=state_dim, d_model=128, step_back=STEP_BACK)
...
class Critic(nn.Module):
    def __init__(self, state_dim, action_dim):
        ...
        self.tanh_em_state = nn.Parameter(torch.ones(STEP_BACK, state_dim) * TANH_EM)
    ...
    def forward(self, state, action):
        state = torch.tanh(self.tanh_em_state * state)
        action = torch.tanh(self.tanh_em_action * action)

        state = self.mylayer(state)
        state = state[:, -1, :]
...
class ReplayMemory:
    ...
    def add_memo(self, state_buffer, action, reward, next_state_buffer, done):
        self.buffer.append(
            (state_buffer, action, reward, next_state_buffer, done)
        )
...
class TD3Agent:
    ...
    def update_networks(self):
        ...
        target_Q1 = self.critic1_target(next_state_buffer_samples, next_action)
        target_Q2 = self.critic2_target(next_state_buffer_samples, next_action)
        target_Q = torch.min(target_Q1, target_Q2)
        target_Q = reward_samples + (GAMMA * target_Q * (1 - done_samples))

        current_Q1 = self.critic1(state_buffer_samples, action_samples)
        current_Q2 = self.critic2(state_buffer_samples, action_samples)
        ...
        if self.total_it % self.policy_delay == 0:
            ...
            actor_loss = -self.critic1(state_buffer_samples, pred_action).mean()
